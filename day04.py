...
# my code
assignments = [x.strip() for x in open(filename).readlines()]
total1 = 0
total2 = 0
for asgn in assignments:
    [ind_asgn1, ind_asgn2] = asgn.split(',')
    [asgn1_start, asgn1_end] = [int(i) for i in ind_asgn1.split("-")]
    [asgn2_start, asgn2_end] = [int(i) for i in ind_asgn2.split("-")]
    if (asgn1_start >= asgn2_start and asgn1_end <= asgn2_end) \
            or (asgn1_start <= asgn2_start and asgn1_end >= asgn2_end):
        total1 += 1
    if (asgn2_start <= asgn1_start <= asgn2_end) \
            or (asgn2_start <= asgn1_end <= asgn2_end) \
            or (asgn1_start <= asgn2_start <= asgn1_end) \
            or (asgn1_start <= asgn2_end <= asgn1_end):
        total2 += 1
