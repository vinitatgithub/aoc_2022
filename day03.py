...
sacks = [x.strip() for x in open(filename).readlines()]
...
for i in range(0, len(sacks), 3):
    ...
    if letter.islower():
        ...
        cnt2 += ord(letter) - 38
print("Part one: " + str(cnt) + " | Part two: " + str(cnt2))
...
with open(filename, 'r') as filehandle:
    ...
    for line in filehandle:
        item_list = list(line)
        compartment1 = item_list[:len(item_list)//2]
        compartment2 = item_list[len(item_list) // 2:]
        common_element = [i for i in compartment1 if i in compartment2]

        if 'a' <= common_element[0] <= 'z':
            item_value = ord(common_element[0]) - 96
        else:
            item_value = ord(common_element[0]) - 64 + 26 # because
...
for i in range(0, len(sacks), 3):
    ...
    c_list = list(c)
    count = 0
    common_element = [i for i in a_list if i in b_list and i in c_list]
    if 'a' <= common_element[0] <= 'z':
        item_value = ord(common_element[0]) - 96
    else:
        item_value = ord(common_element[0]) - 64 + 26 # because
...
print("Part one: " + str(total) + " | Part two: " + str(total2))
