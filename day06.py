...
# my code
input_str = [x.strip() for x in open(filename).readlines()]
...
for i in input_str:
    print("in loop: " + i)
    i_split = list(i)
    for count, element in enumerate(i_split[:-3]):
        if len(set(i_split[count:count+4])) == len(i_split[count:count+4]):
            start_of_packet_marker.append(count + 4)
            break
    for count, element in enumerate(i_split[:-13]):
        if len(set(i_split[count:count+14])) == len(i_split[count:count+14]):
            start_of_message_marker.append(count + 14)
            break
print(f"Part one: {start_of_packet_marker} | Part two: {start_of_message_marker}")